  ...
  return (
    <section className='custom-screen py-20 text-gray-600'>
      <div className='max-w-xl mx-auto text-center'>
        <h2 className='text-gray-800 dark:text-white text-3xl font-semibold sm:text-4xl'>
      ...
      <div className='mt-12 max-w-4xl mx-auto'>
        <ul className='space-y-6 grid-cols-2 gap-6 sm:grid sm:space-y-0 dark:text-gray-400'>
          {featuresList.map((item: Feature, idx: number) => (
            <li key={idx}>
              <a
                href={item.href}
                target='_blank'
                className='block group space-y-3 p-4 rounded-lg border dark:bg-gray-800 dark:hover:bg-gray-700 hover:bg-white from-gray-50 to-gray-100 sm:p-6'>
                <div>{item.icon}</div>
                <h3 className='flex items-center gap-x-0 group-hover:gap-x-3 text-gray-800 text-xl font-semibold duration-200 dark:text-white'>
                  {item.title}
